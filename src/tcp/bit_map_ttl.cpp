...
extern "C" {
	...
	bool mg_bit_map_auth_ttl_update(bit_map_auth_ttl_map *map, uint32_t k, bool forced, uint8_t ttl) {
		uint32_t bucket = (k >> 2) * 5; // division by four rounded down times 5 to determin bucket
		...
		if (ts2 || (forced && map->bucket[bucket + 1 + idx] == ttl)) {
